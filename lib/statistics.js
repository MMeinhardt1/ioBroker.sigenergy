...
class StatisticsCalculator {
    ...
    restoreDayStats(persisted) {
        ...
        if (isValidToday(persisted.batteryDailyChargeTime)) {
            // The exposed value is derived as Math.round((batteryFullTime - (chargeStartTime
            // || dayStart)) / 60000). We don't know the original chargeStartTime, but we can
            // synthesize a batteryFullTime relative to dayStart that reproduces the exact same
            // persisted minute value with chargeStartTime left unset (start = dayStart). This
            // also re-arms the "already completed today" one-shot gate (batteryFullTime !== null),
            // so a stale restart-time SOC reading does not overwrite it with a bogus new duration.
            this._dayStats.batteryFullTime = this._dayStart + persisted.batteryDailyChargeTime.val * 60000;
        }
    }
    ...
    getStats(cd, config) {
        ...
        if (config.calcBatteryDailyFull) {
            if (this._dayStats.batteryFullTime) {
                const start = this._dayStats.chargeStartTime || this._dayStart;
                stats.batteryDailyChargeTime = Math.max(
                    0,
                    Math.round((this._dayStats.batteryFullTime - start) / 60000),
                );
            } else {
                // Not yet fully charged today (e.g. right after midnight reset) —
                // emit null explicitly so the stale value from yesterday gets cleared,
                // instead of omitting the key (which would leave it untouched).
                stats.batteryDailyChargeTime = null;
            }
        }
