...
class StatisticsCalculator {
    ...
    /**
     * Restore today's min/max SOC after an adapter restart.
     * Only applied if the persisted values were last written today — otherwise
     * they belong to a previous day and the freshly initialized defaults
     * (minSoc: 100, maxSoc: 0) are correct as-is.
     *
     * @param {number} minSoc - Persisted statistics.dayMinSoc value
     * @param {number} maxSoc - Persisted statistics.dayMaxSoc value
     * @param {number} lastWrittenTs - Epoch ms timestamp the persisted values were last written
     */
    restoreDayStats(minSoc, maxSoc, lastWrittenTs) {
        if (typeof lastWrittenTs !== 'number' || lastWrittenTs < this._dayStart) {
            return; // Stale (from a previous day) or unknown — keep defaults
        }
        if (typeof minSoc === 'number' && minSoc >= 0 && minSoc <= 100) {
            this._dayStats.minSoc = minSoc;
        }
        if (typeof maxSoc === 'number' && maxSoc >= 0 && maxSoc <= 100) {
            this._dayStats.maxSoc = maxSoc;
        }
    }
